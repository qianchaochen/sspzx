 ...
 * 2017年3月22日
 */
public class TestReqByJsSDKParser extends BaseTestSDKController{
	public void execute() throws Exception {
		//是否采用本地环境还是测试环境
		this.setEnv("test");
		String file_name="TestJsSdkReqEncode.txt";
		
		//构造一个parser,分析file_name日志
		SDKReqParser parser=new SDKReqParser(file_name);
	...
	public static void main(String[] args) {
		TestReqByJsSDKParser t=new TestReqByJsSDKParser();
		try {
			t.execute();
		} catch (Exception e) {
			e.printStackTrace();
